''' 
    Calculates and stores the coefficients for Lasso fitted on each dataset separately with response variable "outcome". 
...
for source in datasets:
    ...
    for random_state in tqdm(range(n_states), desc='Grid Search n_states: '):    
        ...
        X_train = preprocessor.fit_transform(Xy_data[source])
        y_train = Xy_data[source]['outcome']
    ...
    coefs.to_parquet(f'parquet/{outcome}/Individual_Lasso_coefs_{source}_{outcome}.parquet')